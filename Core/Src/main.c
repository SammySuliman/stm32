...
static int16_t g_capture_frame[AUDIO_FRAME_SAMPLES * AUDIO_CHANNELS];
static int16_t g_mono_frame[AUDIO_FRAME_SAMPLES];
static uint8_t g_uart_header[5];
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if ((now_ms - last_blink_ms) >= 500U) {
      last_blink_ms = now_ms;
      HAL_GPIO_TogglePin(LED2_GPIO_Port, LED2_Pin);
    }

    uint32_t seq = audio_capture_latest_sequence();
    if (seq != last_seq && ((seq & 1U) == 0U)) {
      size_t frame_samples = audio_capture_read_latest(g_capture_frame,
                                                       AUDIO_FRAME_SAMPLES);
      if (frame_samples > 0U) {
        for (size_t i = 0; i < frame_samples; ++i) {
          g_mono_frame[i] = g_capture_frame[i * 2U];
        }

        uint16_t payload_bytes = (uint16_t)(frame_samples * sizeof(int16_t));
        g_uart_header[0] = 0x55;
        g_uart_header[1] = 0xAA;
        g_uart_header[2] = (uint8_t)(payload_bytes & 0xFF);
        g_uart_header[3] = (uint8_t)(payload_bytes >> 8);
        g_uart_header[4] = frame_counter++;

        HAL_UART_Transmit(&huart1, g_uart_header, sizeof(g_uart_header), HAL_MAX_DELAY);
        HAL_UART_Transmit(&huart1, (uint8_t *)g_mono_frame, payload_bytes, HAL_MAX_DELAY);
      }
      last_seq = seq;
    }
    /* USER CODE END 3 */
